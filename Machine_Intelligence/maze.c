...
int visit_A_star(int row2, int col2) //通过A*算法寻找迷宫出路
{
    ...
    while (open_list->next != NULL)                                   //通过判断open_list是否为空来跳出循环
    {
         ...
         */
        if (maze[x + 1][(short)y] != 2) //如果此节点不是墙，即可进行添加操作
        {
            if (map[x + 1][(short)y] == 0) //如果当前节点的信息未记录到数组中，就创建新节点并记录
            {
                q = map[x + 1][(short)y] = new_p(x + 1, y, deep + 1);
                add(open_list, q);                   //将节点加入到open_list当中
                q->parent = map[(short)x][(short)y]; //将此节点的父节点设置为当前节点
            }
            else //如果节点已存在
            {
                q = map[x + 1][(short)y]; //取得节点信息
